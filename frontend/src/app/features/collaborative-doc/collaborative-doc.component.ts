...
<div class="page">
  <div class="page-header">
    <h2>📄 Collaborative Document — Field-Level Pessimistic Lock</h2>
    <p class="desc">
    ...
    <div class="arch-note">
      <strong>Flow:</strong> AcquireFieldLock → <code>AuraHub</code> → <code>DocumentLockService</code>
      → broadcast <code>FieldLocked</code> → UI cập nhật realtime
    </div>
  </div>
  ...
  <div class="identity-bar">
    @if (!connected()) {
      <input [(ngModel)]="displayName" placeholder="Tên hiển thị (vd: Alice)" class="input name-input"
      ...
      <div class="you-info">
        <span class="you-badge">{{ displayName }}</span>
        <code class="uid">{{ tenantId }}</code>
      </div>
    ...
    <div class="doc-layout">
      ...
      <div class="locks-panel">
        ...
        @for (lock of activeLocks(); track lock.fieldId) {
          <div class="lock-item" [class.lock-mine]="lock.userId === displayName">
            <div class="lock-field-name">{{ getFieldLabel(lock.fieldId) }}</div>
            <div class="lock-user-name">
              {{ lock.userId === displayName ? '(bạn)' : lock.userName }}
            </div>
...
export class CollaborativeDocComponent implements OnInit, OnDestroy {
  ...
  /** Tenant from the current session — used for JWT token issuance. */
  get tenantId() { return this.sessionSvc.session()!.tenantId; }

  /** Display name the user types — becomes the JWT `name` claim and the lock owner identifier. */
  displayName = this.sessionSvc.session()?.name ?? '';
  ...
  async connect() {
    ...
    try {
      // tenantId comes from the current session; displayName becomes the JWT `name` claim
      // and is used by the hub as the lock owner identifier (UserName claim).
      await this.docService.connect(this.tenantId, name);
  ...
  async onFocus(fieldId: string) {
    ...
    try {
      ...
      if (result.acquired) {
        this.setLock(fieldId, this.displayName, this.displayName, result.expiresAt ?? '');
  ...
  private setLock(fieldId: string, userId: string, userName: string, expiresAt: string) {
    this.lockStates.set(fieldId, {
      ...
      isMyLock:     userId === this.displayName,
