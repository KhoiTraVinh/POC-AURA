...
export class ChatSignalrService {
  ...
  private userReadReceiptSubject = new Subject<{
    staffId: number;
    messageId: number;
  }>();
  ...
  public async startConnection(groupId: number): Promise<void> {
    ...
    this.hubConnection.on(
      "UserReadReceipt",
      (data: { staffId: number; messageId: number }) =>
        this.userReadReceiptSubject.next(data),
    );
  ...
  public async forceClientStop(): Promise<void> {
    if (this.hubConnection) {
      try {
        await this.hubConnection.stop();
      } catch (err) {
        console.error("Error force stopping SignalR connection:", err);
      }
    }
  }

  public updateReadPointer(
    groupId: number,
    staffId: number,
    maxId: number,
  ): void {
    ...
    this.hubConnection
      .invoke("MarkRead", groupId, staffId, maxId)
      .catch((err) =>
        console.error("Failed to update read pointer via Hub:", err),
      );
  }
}
